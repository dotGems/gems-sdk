...
async function main(drop_id: number) {
    ...
    const config = await gems.assets.get_config();
    ...
    const collections = (await gems.assets.get_collections( collection_name ))[0];
    ...
    const templates = await gems.assets.get_templates( collection_name, template_id );
    const { schema_name, immutable_serialized_data } = templates[0];
    console.log(templates[0]);

    // 4. Get Schemas
    const schemas = await gems.assets.get_schemas( collection_name, schema_name );
